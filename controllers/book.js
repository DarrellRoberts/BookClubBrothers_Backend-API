...
const getAllBooks = async (req,res) => {
    try {
        const books = await Book.find().populate("suggestedBy", [
            "name"
        ]);
        if (books.length === 0) {
...
const getOneBook = async (req, res) => {
    ...
    const books = await Book.findOne({ _id: bookId }).populate("suggestedBy", ["name"]);
    if (!books) {
...
const deleteBook = async (req, res) => {
    try {
    const bookId = req.params.bookId;
    const book = await Event.find({_id: bookId});
    if (!book) {
        return res.status(404).json({ msg: "Book not found" });
    }
